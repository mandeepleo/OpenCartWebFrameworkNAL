...
test("POST - Create User", async ({ apiHelper }) => {
  ...
  // GET the user
  let response = await apiHelper.get(`/public/v2/users/${newUserResponse.id}`, AUTH_HEADER);
...
test("PUT - Update User", async ({ apiHelper }) => {
  ...
  // PUT the user
  let response = await apiHelper.put(`/public/v2/users/${newUserResponse.id}`, updateUserData, AUTH_HEADER);
  expect(response.status).toBe(200);
...
test("DELETE - Delete user", async ({ apiHelper }) => {
  ...
  let newUserResponse = await createNewUser(apiHelper);
  // DELETE the user
  let response = await apiHelper.delete(`/public/v2/users/${newUserResponse.id}`, AUTH_HEADER);
